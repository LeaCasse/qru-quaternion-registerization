...
\includegraphics[width=\linewidth]{figures/gauge_only_motion.png}
\centering\small (a)
\end{minipage}\hfill
\begin{minipage}{0.48\columnwidth}
\includegraphics[width=\linewidth]{figures/projection_blindness.png}
\centering\small (b)
\end{minipage}
...
The Bloch-register diagnostic in Fig.~\ref{fig:bloch_register_diagnostic} uses one deterministic depth-eight QRU instance selected by the same identifiability test. For this instance,
\begin{equation}
\begin{aligned}
...
=2.5\times10^{-16}.
\end{equation}
The figure is therefore a checked interface diagnostic, not a supervised-performance result.

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/06_bloch_register_diagnostic.png}
\caption{Bloch-register diagnostic. The selected axis has $2.35\times$ the tangent energy of $e_z$; the three-bit signed register has mean absolute error $5.88\times10^{-2}$.}
...
\begin{equation}
\begin{aligned}
\ket{y}\ket{0}&\longmapsto\ket{y}\ket{\widehat s_y},\\
...
All signed words for $m_s=2,3$ reproduce the direct rotation with state infidelity below $4\times10^{-15}$. A seven-qubit two-input construction maintains a superposition of QRU branches, writes two signed values coherently, applies the downstream evolution, and reverses the pipeline. Its interference echo gives $P(0)=1$ to numerical precision, while interface dephasing gives $P(0)=1/2$.

The complete QAE--decoder--downstream circuit is implemented and transpiled with Qiskit~\cite{Qiskit2024}, then validated against a direct phase-word-controlled reference. For $(m_\phi,m_s)=(4,3)$, it uses ten qubits and attains fidelity above $1-1.4\times10^{-13}$ with signed-register reset probability above $1-1.3\times10^{-13}$. The resources in Table~\ref{tab:resources} are regenerated from the circuit. The finite lookup, together with its uncomputation, is the dominant cost.

\begin{table*}[t]
\centering
\caption{Coherent-interface precision and resources. Transpilation uses basis gates $\{\mathrm{rz},\mathrm{sx},\mathrm{x},\mathrm{cx}\}$, optimization level one, and seed 42. Decoder CX counts one phase-to-signed lookup; the inverse lookup has comparable cost.}
\label{tab:resources}
\begin{tabular}{ccccccc}
\toprule
$(m_\phi,m_s)$ & qubits & depth & CX & decoder CX & grid mean $|\widehat s-s|$ & QRU mean $|\widehat s-s|$ \\
\midrule
$(3,2)$ & 8  & 795   & 326  & 150  & 0.244 & 0.242 \\
$(4,3)$ & 10 & 6156  & 2170 & 1050 & 0.139 & 0.139 \\
$(5,4)$ & 12 & 19443 & 6513 & 3210 & 0.0829 & 0.0816 \\
\bottomrule
\end{tabular}
\end{table*}

The favourable point $p=0.3059$ is not representative. Over the uniform probability grid, only $1.5\%$, $4.5\%$, and $11.4\%$ of values achieve signed error at most $0.02$ for $(3,2),(4,3),(5,4)$. The oscillatory phase-grid error and its downstream effect are shown in Fig.~\ref{fig:results}(b).

\begin{figure}[t]
\centering
\begin{minipage}{0.48\columnwidth}
\includegraphics[width=\linewidth]{figures/readout_generalization.png}
\centering\small (a)
\end{minipage}\hfill
\begin{minipage}{0.48\columnwidth}
\includegraphics[width=\linewidth]{figures/coherent_precision_robustness.png}
\centering\small (b)
\end{minipage}
...
The coherent circuit establishes functional composability, not near-term practicality. A measurement-reinjection pipeline may be preferable for one known classical input. Coherent transport becomes structurally relevant when inputs, scenarios, or downstream branches remain in superposition, as in the two-branch interference test. Even then, standard low-precision QAE has strongly input-dependent errors, while the exact finite decoder scales as a lookup over $2^{m_\phi}$ phase words. A scalable implementation requires a structured reversible approximation of the sine-square map or an estimator producing a more directly consumable numerical representation. The present results therefore support an interface theorem and a verified small demonstrator, not a scalable NISQ architecture.
